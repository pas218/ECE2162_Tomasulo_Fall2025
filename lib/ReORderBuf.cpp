...
bool ReOrderBuf::ableToCommit(int spotNumber)
{
	...
	// std::cout << "Checking commit flag: " << table[spotNumber].cmt_flag << std::endl;
	bool returnVal = table[spotNumber].cmt_flag == 1 ? true : false;
	
	// std::cout << "ReturnVal going into loop: " << (int)returnVal << std::endl;
	/*
	if (headPtr == tailPtr)
	{
		std::cout << "Nothing to commit. The ROB is empty.\n";
	}
	else if (headPtr > tailPtr)
	{
		// Make sure there are no entries before the one we want to commit.
		for (int i = spotNumber-1; i >= tailPtr; i--)
		{
			// std::cout << "table[" << i << "].id: " << table[i].id << std::endl;
			if (table[i].id != -1)
			{
				returnVal = false;
				break;
	...
	{
		std::cout << "TODO: [ReOrderBuf::ableToCommit]: Need to look at the case where tailPtr < headPtr." << std::endl;
	}
	*/
	...
	// std::cout << "ReturnVal coming out of loop: " << (int)returnVal << std::endl;
	return returnVal;
}
commitReturn ReOrderBuf::commit(int spotNumber)
{
	// std::cout << "Commiting ROB spot #" << spotNumber << std::endl;
